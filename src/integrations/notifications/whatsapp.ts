...
export interface WhatsAppNumberValidationResult {
  jid?: string;
  exists: boolean;
  number: string;
  name?: string;
...
export async function sendWhatsAppMessage(
  number: string,
  text: string,
  config: WhatsAppConfig | null = getWhatsAppConfigFromEnv(),
): Promise<void> {
  ...
  const response = await fetch(
    `${config.baseUrl.replace(/\/$/, "")}/message/sendText/${config.instance}`,
    {
      method: "POST",
      headers: {
...
export async function validateWhatsAppNumbers(
  numbers: string[],
  config: WhatsAppConfig | null = getWhatsAppConfigFromEnv(),
): Promise<WhatsAppNumberValidationResult[] | null> {
  if (!config) {
    return null;
  }
  ...
  const response = await fetch(
    `${config.baseUrl.replace(/\/$/, "")}/chat/whatsappNumbers/${config.instance}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    throw new Error(
      `WhatsApp number validation failed: HTTP ${response.status} ${response.statusText} | Response: ${typeof body === "string" ? body : JSON.stringify(body)}`,
    );
  }
...
export async function getWhatsAppConnectionState(
  config: WhatsAppConfig | null = getWhatsAppConfigFromEnv(),
): Promise<WhatsAppConnectionState | null> {
  if (!config) {
    return null;
  }

  const response = await fetch(
    `${config.baseUrl.replace(/\/$/, "")}/instance/connectionState/${config.instance}`,
    {
      method: "GET",
      headers: {
        apikey: config.apiKey,
  ...
  if (!response.ok) {
    throw new Error(
      `WhatsApp connection state failed: HTTP ${response.status} ${response.statusText}`,
    );
  }
