...
def get_headless_mode() -> bool:
    raw_value = (os.getenv("CLOAKBROWSER_HEADLESS") or "").strip().lower()
...
def wait_until_title_is_not_cloudflare(page, timeout_seconds: float) -> str:
    start = time.time()
    last_title = ""
    ...
    while time.time() - start < timeout_seconds:
        try:
            ...
            if "Execution context was destroyed" in message:
                log("Title check interrupted by navigation; retrying.")
...
def main() -> None:
    ...
    try:
        ...
        wait_until_title_is_not_cloudflare(page, timeout_seconds=timeout_seconds)
        log("Cloudflare wait page cleared.")

        cookies = page.context.cookies([TARGET_URL])
        cookie_header = build_cookie_header(cookies)
        if not cookie_header:
            raise RuntimeError("Title changed, but no cookies were available.")
